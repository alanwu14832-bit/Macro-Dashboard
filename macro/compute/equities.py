...
# ---- AI 供應鏈熱力圖 ------------------------------------------------------
# 上中下游一條龍：上游晶片 → 中游零組件（散熱、電源、連接、載板、機構）
# → 下游組裝與網通。跟半導體鏈刻意分開：那邊看的是製程環節，這邊看的
# 是「AI 資本支出流到台灣的路徑」——同一天上游漲、中游散熱跌，說的是
# 訂單預期與毛利分配的故事，不是製程的故事。
TW_AI_CHAIN = [
    ("上游｜晶片與先進封裝", [("2330", "台積電"), ("2454", "聯發科"),
                              ("3443", "創意"), ("3661", "世芯-KY"),
                              ("3711", "日月光投控")]),
    ("中游｜散熱", [("3017", "奇鋐"), ("3324", "雙鴻"), ("2421", "建準"),
                    ("3653", "健策"), ("8996", "高力")]),
    ("中游｜電源", [("2308", "台達電"), ("2301", "光寶科"), ("6409", "旭隼")]),
    ("中游｜連接與線纜", [("3023", "信邦"), ("3533", "嘉澤"), ("3665", "貿聯-KY")]),
    ("中游｜載板與 PCB", [("3037", "欣興"), ("8046", "南電"), ("2368", "金像電"),
                          ("2383", "台光電")]),
    ("中游｜機殼與導軌", [("8210", "勤誠"), ("2059", "川湖"), ("3013", "晟銘電")]),
    ("下游｜伺服器組裝與品牌", [("2317", "鴻海"), ("2382", "廣達"), ("3231", "緯創"),
                                ("6669", "緯穎"), ("2356", "英業達"),
                                ("2376", "技嘉"), ("3706", "神達")]),
    ("下游｜網通與光通訊", [("2345", "智邦"), ("6285", "啟碁"), ("3596", "智易"),
                            ("6442", "光聖"), ("4977", "眾達-KY")]),
]
...
def compute(bundle=None) -> dict:
    ...
    heat_pairs = list({symbol: (symbol, name)
                       for _, pairs in TW_GROUPS + TW_AI_CHAIN + TW_SEMI_CHAIN
