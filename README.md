...
**收盤後自動放慢。** 連兩輪報價完全沒有變化（收盤、週末、假日）就把
輪詢間隔加倍，一路放慢到台股 90 秒、美股 5 分鐘；一偵測到變化立刻
回到基本節奏。台股一天只有 4.5 小時盤中，這把其餘時間的呼叫量砍掉
九成以上，而且不用維護交易日曆——假日自己會慢下來。分頁切到背景時
則完全停止輪詢。
...
**掃描發票與條碼。** 記一筆的表單裡有掃描鈕。取景器自己拿 getUserMedia 的
串流（要求 1080p），每 120ms 把框內那塊以原生解析度畫到 canvas 交給 ZXing
（`zxing.min.js`，按了才載入；Android Chrome 有原生 BarcodeDetector 就用它）。
不用現成的掃描元件是因為它們把影像縮到螢幕尺寸才解——發票那顆一公分多、
五十幾個模組的 QR 在 390px 寬的螢幕上一格剩不到一個像素，永遠解不開。
